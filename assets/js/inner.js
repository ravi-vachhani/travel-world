...
const WA_NUMBER = "919904040001"; // WhatsApp number (no + sign)
...
function buildWhatsAppURL(form) {
  ...
  if (path.includes("/destinations/")) {
    ...
    context = slug
      ? `Destination Enquiry: ${slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}`
      : "Destination Enquiry";
  } else if (path.includes("/services/")) {
    const slug = path.split("/services/")[1]?.replace(/\//g, "") || "";
    context = slug
      ? `Service Enquiry: ${slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}`
      : "Service Enquiry";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll(
      ".btn-submit:not(form .btn-submit), a.btn-enquire, a[data-wa]",
    )
    .forEach((btn) => {
      btn.addEventListener("click", (e) => {
        if (btn.tagName === "A" && btn.href && !btn.href.includes("wa.me")) {
          ...
          e.preventDefault();
          window.open(
            `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent("Hello, I would like to enquire about a trip.")}`,
            "_blank",
          );
        }
      });
